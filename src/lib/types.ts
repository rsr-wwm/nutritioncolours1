...
export interface TeamMember {
  id: string;
  slug?: string; // URL slug for /team/[slug]; falls back to id if unset
  name: string;
  role: string;
...
export interface Recipe {
  ...
  preparedBy?: string;
  instructions?: string[]; // step-by-step prep instructions; optional — schema falls back to []
  glycemicLoad?: string; // e.g. "Low"/"Medium"/"High"; optional — templates fall back to "Low"
  pageTitle?: string;
  metaDescription?: string;
  primaryKeyword?: string;
  aeoDirectSnippet?: string;
...
export interface Plan {
  ...
  icon?: string;
  color?: string;
  duration?: string; // e.g. "12 weeks" — optional; templates fall back to "Flexible" when absent
}
...
export interface HerbalEntity {
  ...
  aeoDirectSnippet?: string;
  // Added optional fields for Herb page usage
