...
/**
 * Build the entity graph for a nutrigenomics variant page. Models the gene as a
 * schema.org `Gene` (health-lifesci) with a dbSNP `sameAs`, plus a MedicalWebPage
 * (reviewer metadata) and a Claim carrying the variant's real, PMID-cited clinical
 * impact. Every field is drawn from GENOMIC_VARIANTS_DATA — nothing invented.
 */
function buildGenomicVariantGraph(variant: any): any[] {
  const route = `/genomics/${variant.id}`;
  const gene = {
    '@type': 'Gene',
    '@id': `${BASE}${route}#gene`,
    'name': variant.geneName,
    ...(variant.rsid && { 'alternateName': variant.rsid }),
    'url': `${BASE}${route}`,
    'description': variant.clinicalImpact || '',
    ...(variant.rsid && { 'sameAs': [`https://www.ncbi.nlm.nih.gov/snp/${variant.rsid}`] }),
  };
  const citations = (variant.citations || []).map((c: { text: string; url: string }) => ({
    '@type': 'CreativeWork', 'name': c.text, 'url': c.url,
  }));
  const claim = {
    '@type': 'Claim',
    '@id': `${BASE}${route}#claim`,
    'name': `Nutrigenomic guidance: ${variant.geneName} ${variant.genotype || ''}`.trim(),
    ...(variant.clinicalImpact && { 'description': variant.clinicalImpact }),
    'about': { '@id': `${BASE}${route}#gene` },
    'claimInterpreter': { '@id': `${BASE}/#organization` },
    'appearance': { '@id': `${BASE}${route}#page` },
    'firstAppearance': { '@id': `${BASE}${route}#page` },
    ...(citations.length > 0 && { citation: citations }),
  };
  const webpage = buildMedicalWebPage(route, {
    title: `${variant.rsid} (${variant.genotype}) — Nutrigenomics`,
    description: variant.clinicalImpact,
    reviewType: variant.geneName,
  });
  return [ORG_SCHEMA, PERSON_SCHEMA, webpage, gene, claim];
}

/**
 * Generate the nutrigenomics variant entity graph as an injectable JSON-LD string.
 */
export function getGenomicsEntityGraph(variant: any): string {
  return JSON.stringify({ '@context': 'https://schema.org', '@graph': buildGenomicVariantGraph(variant) });
}

/**
 * Build the entity graph for a drug–nutrient interaction page. Models the drug as a
 * schema.org `Drug` (health-lifesci) with `drugClass` and a `foodWarning` summarizing
 * real depleted nutrients, plus a MedicalWebPage and a Claim carrying the real,
 * PMID-cited monitoring advice. Every field is drawn from DRUG_INTERACTIONS_DATA.
 */
function buildDrugInteractionGraph(interaction: any): any[] {
  const route = `/interactions/${interaction.id}`;
  const drug = {
    '@type': 'Drug',
    '@id': `${BASE}${route}#drug`,
    'name': interaction.drugName,
    'url': `${BASE}${route}`,
    ...(interaction.drugClass && { 'drugClass': { '@type': 'DrugClass', 'name': interaction.drugClass } }),
    ...(interaction.depletedNutrients?.length > 0 && {
      'foodWarning': `May deplete or interact with: ${interaction.depletedNutrients.join('; ')}`,
    }),
  };
  const citations = (interaction.citations || []).map((c: { text: string; url: string }) => ({
    '@type': 'CreativeWork', 'name': c.text, 'url': c.url,
  }));
  const claim = {
    '@type': 'Claim',
    '@id': `${BASE}${route}#claim`,
    'name': `Drug–nutrient interaction: ${interaction.drugName}`,
    ...(interaction.monitoringAdvice && { 'description': interaction.monitoringAdvice }),
    'about': { '@id': `${BASE}${route}#drug` },
    'claimInterpreter': { '@id': `${BASE}/#organization` },
    'appearance': { '@id': `${BASE}${route}#page` },
    'firstAppearance': { '@id': `${BASE}${route}#page` },
    ...(citations.length > 0 && { citation: citations }),
  ...
  const webpage = buildMedicalWebPage(route, {
    title: `${interaction.drugName} — Drug–Nutrient Interaction`,
    description: interaction.monitoringAdvice,
    reviewType: interaction.drugName,
  });
  return [ORG_SCHEMA, PERSON_SCHEMA, webpage, drug, claim];
}

/**
 * Generate the drug–nutrient interaction entity graph as an injectable JSON-LD string.
 */
export function getInteractionEntityGraph(interaction: any): string {
  return JSON.stringify({ '@context': 'https://schema.org', '@graph': buildDrugInteractionGraph(interaction) });
}

/**
 * Build the entity graph for a "topic" (a disease/condition landing page — per the
 * 2026 owner ruling that Health Topics ARE diseases). Brings topic pages to parity
 * with condition pages: a MedicalWebPage carrying a real named Physician reviewer +
 * lastReviewed (from the topic's own `clinicalReview`), a MedicalCondition entity,
 * and — when the topic has citations — a graded Claim. Organization/Person nodes are
 * intentionally omitted here (BaseLayout already emits them site-wide) to avoid
 * duplicate top-level nodes. FAQ is emitted separately via getFaqPageSchema.
 */
export function getTopicEntityGraph(topic: any): string {
  const route = `/topic/${topic.id}`;
  const webpage = buildMedicalWebPage(route, {
    title: topic.pageTitle || topic.title,
    description: topic.metaDescription || topic.shortDesc || topic.solution,
    lastReviewed: topic.clinicalReview?.lastUpdated,
    reviewType: topic.title,
  });
  const condition = {
    '@type': 'MedicalCondition',
    '@id': `${BASE}${route}#condition`,
    'name': topic.title,
    'url': `${BASE}${route}`,
    'description': topic.shortDesc || topic.solution || '',
    'possibleTreatment': { '@type': 'MedicalTherapy', 'name': 'Circadian Nutrition Optimization' },
    'about': { '@id': `${BASE}/#organization` },
  };
  const graph: any[] = [webpage, condition];

  const citations = (topic.citations || []).map((c: { text: string; url: string }) => ({
    '@type': 'CreativeWork', 'name': c.text, 'url': c.url,
  }));
  if (topic.solution || citations.length > 0) {
    graph.push({
      '@type': 'Claim',
      '@id': `${BASE}${route}#claim`,
      'name': `Clinical protocol: ${topic.title}`,
      ...(topic.solution && { 'description': topic.solution }),
      'about': { '@id': `${BASE}${route}#condition` },
      'claimInterpreter': { '@id': `${BASE}/#organization` },
      'appearance': { '@id': `${BASE}${route}#page` },
      'firstAppearance': { '@id': `${BASE}${route}#page` },
      ...(citations.length > 0 && { citation: citations }),
...
/**
 * Build the entity graph for a food page (fruit/vegetable/etc — MRS §13–§25).
 * MedicalWebPage (named Physician reviewer) + a food Thing carrying per-100g composition
 * as additionalProperty (sourced to USDA/IFCT) + a graded Claim. All values come from the
 * food record; nothing invented.
 */
export function getFoodEntityGraph(food: any, classSlug: string): string {
  const route = `/foods/${classSlug}/${food.id}`;
  const webpage = buildMedicalWebPage(route, {
    title: `${food.name} — nutrition, composition & health`,
    description: food.summary,
    lastReviewed: food.lastReviewed,
    reviewType: food.name,
  });
  const composition = (food.composition || []).map((n: { name: string; amount: number; unit: string }) => ({
    '@type': 'PropertyValue', 'name': n.name, 'value': n.amount, 'unitText': n.unit,
  }));
  const thing = {
    '@type': 'Thing',
    '@id': `${BASE}${route}#food`,
    'name': food.name,
    ...(food.scientificName && { 'alternateName': food.scientificName }),
    'url': `${BASE}${route}`,
    'description': food.summary,
    ...(composition.length > 0 && { 'additionalProperty': [
      { '@type': 'PropertyValue', 'name': 'Energy', 'value': food.energyKcal, 'unitText': 'kcal per 100 g' },
      ...(food.glycemicIndex != null ? [{ '@type': 'PropertyValue', 'name': 'Glycemic Index', 'value': food.glycemicIndex }] : []),
      ...composition,
    ] }),
    'about': { '@id': `${BASE}/#organization` },
  };
  const sources = (food.sources || []).map((c: { text: string; url: string }) => ({
    '@type': 'CreativeWork', 'name': c.text, 'url': c.url,
  }));
  const graph: any[] = [webpage, thing];
  const topNote = (food.healthNotes || [])[0];
  if (topNote) {
    graph.push({
      '@type': 'Claim',
      '@id': `${BASE}${route}#claim`,
      'name': `${food.name}: nutrition claim`,
      'description': topNote.claim,
      'about': { '@id': `${BASE}${route}#food` },
      'claimInterpreter': { '@id': `${BASE}/#organization` },
      'appearance': { '@id': `${BASE}${route}#page` },
