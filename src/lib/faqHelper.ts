...
export const getDynamicCategoryFaqs = (path: string, activeLocation: any): { question: string; answer: string; category: string }[] => {
  ...
  if (base === 'plans') {
    ...
    if (id === 'cellular-resurrection') {
      return [
        { question: "Is the Cellular Resurrection program suitable for managing long-term Type 2 Diabetes?", answer: "Yes, this intensive program is specifically designed to support insulin sensitivity and pancreatic health under the direct oversight of Dr. Shilpa Thakur.", category: "Remission" },
        ...
        { question: "Can I reduce dependence on medications under clinical supervision?", answer: "Many patients with Type 2 Diabetes successfully transition off medications under their doctor's supervision after achieving metabolic stability with our protocol.", category: "Goal" }
      ];
    }
    return PLANS_FAQS;
  }
