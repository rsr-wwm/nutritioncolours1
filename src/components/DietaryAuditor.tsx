...
import { IconBot, IconX, IconFileText, IconMapPin } from './ui/Icons';
...
const DietaryAuditorInner = ({ activeLocation }: { activeLocation?: any }) => {
  ...
  const runAudit = async () => {
    ...
    try {
      ...
      if (response.text) {
        ...
        if (parsed) setAuditReport(parsed);
      }
    } catch (e) {
      logger.error('DietaryAuditor', 'Safety audit generation failed', e);
...
export const DietaryAuditor = (props: any) => (
  <ViewerTrackerProvider>
    <DietaryAuditorInner {...props} />
  </ViewerTrackerProvider>
);
