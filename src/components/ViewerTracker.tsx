...
import { obfuscatedStorage } from '@/lib/obfuscatedStorage';
...
export const ViewerTrackerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [consent, setConsent] = useState<'granted' | 'declined' | 'pending'>('pending');
  const currentSessionRef = useRef<ViewerSession | null>(null);

  // Initialize consent preference
  useEffect(() => {
    const storedConsent = obfuscatedStorage.getItem('nutrition_tracker_consent');
  ...
  // Initialize tracking data from obfuscatedStorage
  useEffect(() => {
    ...
    const savedHistory = obfuscatedStorage.getItem('nutrition_tracker_history');
    const parsedHistory: ViewerSession[] = safeJsonParse<ViewerSession[]>(savedHistory, []);
    setSessionsHistory(parsedHistory);

    // 2. Load historical reports
    const savedReports = obfuscatedStorage.getItem('nutrition_tracker_report_history');
    const parsedReports: DailyAnalyticsReport[] = safeJsonParse<DailyAnalyticsReport[]>(savedReports, []);
    setReportsHistory(parsedReports);

    // 3. Setup/Resume active session
    const savedActive = obfuscatedStorage.getItem('nutrition_tracker_current_session');
    let session: ViewerSession;

    if (savedActive) {
      const parsedActive: ViewerSession | null = safeJsonParse<ViewerSession | null>(savedActive, null);
      if (!parsedActive) {
        ...
        if (isExpired) {
          ...
          obfuscatedStorage.setItem('nutrition_tracker_history', JSON.stringify(updatedHistory));
    ...
    obfuscatedStorage.setItem('nutrition_tracker_current_session', JSON.stringify(session));

    // 4. Automated daily report check is handled by useInterval below.
  ...
  // 4. Automated daily report check — declarative interval (no stale closures)
  useInterval(() => {
    if (consent !== 'granted') return;
    const storedLastReport = obfuscatedStorage.getItem('nutrition_tracker_last_report_time');
    const lastSent = storedLastReport ? parseInt(storedLastReport) : 0;
    if (Date.now() - lastSent > DAILY_INTERVAL) {
      const report = autoCompileAndSendReport();
      if (report) {
        logger.info('ViewerTracker', 'Automated daily analytical report transmitted', { report });
      }
    }
  }, 60000);
  ...
  const createNewSession = (): ViewerSession => {
    ...
    return {
      ...
      deviceProfile: {
        ...
        platform: (navigator as any).userAgentData?.platform || 'WebBrowser'
      },
      interactions: [
        {
          timestamp: Date.now(),
  ...
  const trackInteraction = (type: UserInteraction['type'], description: string, pathOverride?: string) => {
    ...
    obfuscatedStorage.setItem('nutrition_tracker_current_session', JSON.stringify(updatedSession));
  };

  const clearTrackingData = () => {
    obfuscatedStorage.removeItem('nutrition_tracker_history');
    obfuscatedStorage.removeItem('nutrition_tracker_report_history');
    obfuscatedStorage.removeItem('nutrition_tracker_last_report_time');
    obfuscatedStorage.removeItem('nutrition_clinic_appointments');
    obfuscatedStorage.removeItem('nutrition_inquiries');
    obfuscatedStorage.removeItem('nutrition_saved_plans');
    obfuscatedStorage.removeItem('VITE_GEMINI_API_KEY');
    setSessionsHistory([]);
    setReportsHistory([]);
    if (consent === 'granted') {
      ...
      obfuscatedStorage.setItem('nutrition_tracker_current_session', JSON.stringify(freshSession));
    } else {
  ...
  const compileDailyReport = (forceRecipient?: string): DailyAnalyticsReport => {
    ...
    const active: ViewerSession | null = safeJsonParse<ViewerSession | null>(savedActive, null);
    const history: ViewerSession[] = safeJsonParse<ViewerSession[]>(savedHistory, []);
    ...
    const disclosureReach: Record<string, number> = {};
    ...
    allSessions.forEach(session => {
      session.interactions.forEach(evt => {
        ...
        } else if (evt.type === 'disclosure') {
          disclosureReach[evt.description] = (disclosureReach[evt.description] || 0) + 1;
    ...
    const searchKeywords = Object.entries(searches).map(([k, v]) => ({ keyword: k, count: v }));
    const systemHealthScore = Math.max(0, 100 - errorLogs.length * 10);
    const recipient = forceRecipient || 'dr.shilpa.thakur.clinic@gmail.com';
    const htmlReportSnippet = '';
  ...
  const sendEmailReport = async (report: DailyAnalyticsReport): Promise<boolean> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const currentReports = obfuscatedStorage.getItem('nutrition_tracker_report_history');
        const parsedReports: DailyAnalyticsReport[] = safeJsonParse<DailyAnalyticsReport[]>(currentReports, []);
        const updated = [report, ...parsedReports].slice(0, 30); // keep last 30 daily reports
        
        obfuscatedStorage.setItem('nutrition_tracker_report_history', JSON.stringify(updated));
        obfuscatedStorage.setItem('nutrition_tracker_last_report_time', Date.now().toString());
  ...
  const autoCompileAndSendReport = (): DailyAnalyticsReport | null => {
    try {
      ...
      logger.error('ViewerTracker', 'Failed to autocompile and send report', err);
      return null;
    }
  };

  const handleConsentResponse = (response: 'granted' | 'declined') => {
    obfuscatedStorage.setItem('nutrition_tracker_consent', response);
    setConsent(response);
  };
    ...
    <ViewerTrackerContext.Provider value={{
      ...
      {consent === 'pending' && (
        <div 
          ...
          className="md:flex-row md:items-center md:justify-between md:gap-8"
        >
          <div style={{ flex: 1 }}>
            <h4 style={{ margin: '0 0 6px 0', fontSize: '15px', fontWeight: '700', color: '#10b981' }}>
              Privacy & Analytics Preferences
            </h4>
            <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8', lineHeight: '1.5' }}>
              We use secure, localized, and obfuscated analytical tracking to optimize our clinical content and layouts. No personal identification data is ever collected or shared.
            </p>
          </div>
          <div style={{ display: 'flex', gap: '12px', flexShrink: 0 }}>
            <button 
              onClick={() => handleConsentResponse('declined')}
              style={{
            ...
            <button 
              onClick={() => handleConsentResponse('granted')}
              style={{
                background: '#10b981',
                border: 'none',
                color: '#0f172a',
