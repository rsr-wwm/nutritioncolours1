...
import { LOCATIONS_DATA, LocationNode } from '@/lib/locationsData';
import { INTERNATIONAL_COUNTRIES, InternationalCountryNode } from '@/lib/internationalData';
...
export const LocalDirectory: React.FC<LocalDirectoryProps> = ({ navigate, currentPath }) => {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      logger.error('LocalDirectory', 'PoW verification failed', err);
    }
                ...
                {bookingSuccess ? (
                  ...
                  <form onSubmit={handleBookingSubmit} className="space-y-4">
                    ...
                    <button 
                      ...
                      className="bg-lime-400 hover:bg-lime-300 disabled:bg-emerald-900/60 disabled:text-emerald-500/80 text-emerald-950 font-black uppercase tracking-wider text-xs py-4 rounded-xl transition-all shadow-md cursor-pointer w-full mt-4 flex items-center justify-center gap-2"
                    >
                      {bookingSubmitting ? (
