    ...
    <div className="py-24 animate-in fade-in bg-stone-50/20">
      ...
      {activeTab === 'hub' ? (
        ...
        <div className="max-w-6xl mx-auto px-6 space-y-12 animate-in fade-in slide-in-from-bottom-8">
          ...
          <div className="card-standard p-4 rounded-2xl flex items-center justify-between shadow-sm">
          ...
          <div className="card-standard space-y-4">
            ...
            <div className="flex gap-4">
              <input
                ...
                value={newPostAuthor}
                onChange={(e) => setNewPostAuthor(e.target.value)}
                className="form-input text-xs font-bold w-1/4 bg-stone-50 border border-stone-100"
              />
              <input
                type="text"
                ...
                value={newPostText}
                onChange={(e) => setNewPostText(e.target.value)}
                className="flex-1 form-input text-xs bg-stone-50 border border-stone-100"
              />
              <button
                onClick={(e) => {
                ...
                className="btn-primary px-6 py-3 text-[9px]"
              >
                Post
              </button>
            </div>
          ...
          <div className="space-y-6">
            {posts.filter(p => {
              ...
              posts.filter(p => {
                ...
                <div key={post.id} className="card-standard flex flex-col text-left">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="badge-clinical text-[8px] px-2 py-0.5">
                            {post.condition}
                          </span>
                  ...
                  {post.comments.length > 0 && (
                    <div className="space-y-3 bg-canvas rounded-2xl p-4 mt-4 border border-default">
                      {post.comments.map((comment, cIdx) => (
                  ...
                  {showCommentInput[post.id] && (
                    <div className="flex gap-2 items-center mt-3">
                      <input 
                        ...
                        value={newCommentText[post.id] || ''}
                        onChange={(e) => setNewCommentText(prev => ({ ...prev, [post.id]: e.target.value }))}
                        className="flex-1 form-input text-xs bg-stone-50 border border-default"
                      />
                      <button 
                        onClick={() => handleAddComment(post.id)}
                        className="btn-primary text-[8px] px-3 py-2 rounded-lg"
                      >
                        Post
                      </button>
                    </div>
