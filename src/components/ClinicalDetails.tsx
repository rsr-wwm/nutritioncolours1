    ...
    return (
      <div className="clinical-summary-card">
        ...
        <div className="summary-grid">
          <div className="summary-item"><span className="item-label">Clinical Class</span><span className="item-value">{herb.name}, scientifically {herb.scientificName}, is a clinical-grade {herb.category} containing {herb.activeCompounds.join(', ')}.</span></div>
...
export const ChrononutritionTimeline = () => {
  ...
  return (
    <div className="bg-canvas border border-default rounded-[32px] p-6 md:p-8 space-y-8 relative overflow-hidden">
      ...
      <div className="card-standard grid md:grid-cols-12 gap-6 items-center">
        <div className="md:col-span-5 space-y-3">
          ...
          <p className="badge-clinical font-mono text-[10px]">{m.hormone}</p>
        </div>
...
export const BiomarkerRiskTracker = ({ biomarkersList }: { biomarkersList: string[] }) => {
  ...
  return (
    <div className="bg-canvas border border-default rounded-[32px] p-6 md:p-8 space-y-8">
      ...
      <div className="space-y-6">
        {specs.map(spec => { const v = values[spec.id] ?? spec.startValue; const s = getStatus(v, spec); return (
          <div key={spec.id} className="card-standard space-y-4">
            ...
            <div className="p-4 bg-canvas border border-default rounded-2xl text-xs flex gap-3 items-start"><span className="text-emerald-700 mt-0.5">ℹ</span><p className="text-stone-600 leading-relaxed"><strong>Protocol:</strong> {spec.remedy}</p></div>
          </div>
        ); })}
      </div>
    </div>
...
export const RelatedContent = ({ currentPath, navigate }: { currentPath: string; navigate: (path: string) => void }) => {
  ...
  if (base === 'herb') {
    ...
    if (id === 'mthfr-c677t') {
      constellationNodes = [
        { id: 'gut-health', label: 'Gut Microbiome', href: '/condition/gut-health', category: 'condition' as const },
        { id: 'metformin-interaction', label: 'Metformin Profile', href: '/interactions/metformin-interaction', category: 'condition' as const },
        { id: 'garlic', label: 'Garlic (Allium sativum)', href: '/herb/garlic', category: 'herb' as const }
      ];
    ...
    } else if (id === 'levothyroxine-interaction') {
      constellationNodes = [
        { id: 'thyroid-dysfunction', label: 'Hypothyroidism', href: '/condition/thyroid-dysfunction', category: 'condition' as const },
        { id: 'ashwagandha', label: 'Ashwagandha Adaptogen', href: '/herb/ashwagandha', category: 'herb' as const }
      ];
    } else {
      constellationNodes = [
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-12 gap-12 mb-12 items-start">
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center gap-3 flex-wrap">
              <span className="badge-clinical">Clinical {herb.category} Profile</span>
              <span className="text-xs font-mono text-stone-500 italic">{herb.scientificName}</span>
              <a href="#clinical-evidence-table" className="badge-clinical hover:bg-brand-100 transition-colors">
                🧪 Evidence Grade: I (PubMed Verified)
              </a>
            </div>
            ...
            {herb.alsoKnownAs && herb.alsoKnownAs.length > 0 && (<section aria-label="Entity Disambiguation" className="mt-4 card-standard"><p className="text-xs font-black uppercase tracking-widest text-stone-500 mb-2">Also Known As</p><p className="text-sm text-stone-700 font-semibold">{herb.alsoKnownAs.join(', ')}</p>{herb.plantFamily && <p className="text-xs text-stone-500 mt-2"><span className="font-bold">Family:</span> {herb.plantFamily} | <span className="font-bold">Species:</span> <em>{herb.scientificName}</em> | <span className="font-bold">Origin:</span> {herb.origin}</p>}</section>)}
            <div className="p-6 bg-accent border border-default rounded-3xl space-y-2 mt-4"><span className="text-[10px] font-black uppercase tracking-widest text-emerald-800 block">Direct Answer Summary</span><dfn className="not-italic text-sm text-stone-600 block leading-relaxed font-semibold">{getAeoSnippet(herb.aeoDirectSnippet, herb.primaryMechanism)}</dfn></div>
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            {activeTab === 'overview' && (<div className="animate-in fade-in card-premium p-8 md:p-12 space-y-8">
              <h2 className="text-2xl font-black text-emerald-950 brand-font">Protocol Parameters</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><span className="text-[10px] font-black uppercase tracking-widest text-stone-500">Therapeutic Dosage</span><p className="text-base font-bold text-emerald-950">{herb.dosage.range}</p><p className="text-xs text-stone-500">{herb.dosage.instruction}</p></div>
                <div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><span className="text-[10px] font-black uppercase tracking-widest text-stone-500">Synergy Partners</span><div className="flex flex-wrap gap-2 pt-1">{herb.synergies.map(s => <span key={s} className="px-3 py-1 bg-stone-100 text-stone-600 rounded-lg text-xs font-semibold">{s}</span>)}</div></div>
              </div>
            ...
            {activeTab === 'science' && (<div className="animate-in fade-in card-premium p-8 md:p-12 space-y-8">
              <h2 className="text-2xl font-black text-emerald-950 brand-font">Mechanism of Action</h2>
              <div className="p-6 bg-accent border border-default rounded-3xl text-stone-600 dark:text-stone-300 text-sm leading-relaxed">{herb.primaryMechanism}</div>
              <div className="space-y-4"><h3 className="text-xs font-black uppercase tracking-widest text-stone-500">Bio-Active Compounds</h3><div className="grid grid-cols-2 md:grid-cols-3 gap-4">{herb.activeCompounds.map(c => (<div key={c} className="p-4 bg-canvas border border-default rounded-2xl text-center"><span className="text-xs font-bold text-emerald-950">{c}</span></div>))}</div></div>
              <div id="clinical-evidence-table" className="space-y-4 border-t border-stone-100 pt-6"><h3 className="text-xs font-black uppercase tracking-widest text-stone-500">References</h3><ul className="space-y-3">{herb.citations.map((c, idx) => (<li key={c.text} className="text-xs text-stone-500 flex items-start gap-2"><span className="font-mono text-emerald-700">[{idx + 1}]</span><a href={c.url} target="_blank" rel="noopener noreferrer" className="hover:text-emerald-800 underline">{c.text}</a></li>))}</ul></div>
            </div>)}
            {activeTab === 'faqs' && (<div className="animate-in fade-in space-y-6">{(herb.faqs || []).map(faq => (<div key={faq.question} className="card-standard space-y-3"><div className="text-lg font-bold text-emerald-950 mb-3">{faq.question}</div><p className="text-sm text-stone-600 leading-relaxed">{faq.answer}</p></div>))}</div>) }
          </div>
          <div className="lg:col-span-4 space-y-6">
            <div className="card-standard p-4 rounded-[40px]"><div className="aspect-[4/3] rounded-[28px] overflow-hidden mb-6 bg-emerald-950 relative flex items-center justify-center"><IconLeaf className="w-24 h-24 text-lime-400/30" /></div><div className="p-4"><p className="text-xs text-stone-500 leading-relaxed mb-6">This therapeutic is configured inside custom patient plans.</p><CompatLink href="/plans" className="w-full btn-primary py-4 text-center block">Get Custom Protocol</CompatLink></div></div>
            <div className="card-standard"><div className="text-xs font-black uppercase tracking-[0.2em] text-stone-500 mb-4 pl-2">Treats Conditions</div><div className="space-y-3">{conditionsData.filter(cond => cond.therapeuticSpices.some(s => s.toLowerCase().includes(herb.name.toLowerCase()) || herb.name.toLowerCase().includes(s.toLowerCase()))).map(cond => (<CompatLink href={`/condition/${cond.id}`} key={cond.id} className="group cursor-pointer p-4 bg-canvas hover:bg-accent border border-default rounded-2xl transition-all flex justify-between items-center block"><div><div className="font-bold text-emerald-950 leading-tight group-hover:text-emerald-700 transition-colors">{cond.name}</div><p className="text-[9px] font-black text-stone-500 uppercase tracking-widest mt-1">Remission Protocol</p></div><IconArrowRight size={14} className="text-stone-500 group-hover:text-emerald-700 transition-all" /></CompatLink>))}</div></div>
          </div>
        </div>
    ...
    <div className="animate-in fade-in duration-700 min-h-screen bg-stone-50/30 pb-24 pt-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-12 gap-12 mb-12 items-start">
          <div className="lg:col-span-8 space-y-6">
            <div className="flex items-center gap-3 flex-wrap">
              <span className="badge-clinical inline-block">Clinical Reversal Protocol</span>
              <a href="#clinical-evidence-table" className="badge-clinical hover:bg-brand-100 transition-colors">
                🧪 Evidence Grade: I (PubMed Verified)
              </a>
            </div>
            ...
            <div className="p-6 bg-accent border border-default rounded-3xl space-y-2"><span className="text-[10px] font-black uppercase tracking-widest text-emerald-800 block">Direct Answer Summary</span><dfn className="not-italic text-sm text-stone-600 block leading-relaxed font-semibold">{getAeoSnippet(cond.aeoDirectSnippet, cond.rootCause)}</dfn></div>
            <ClinicalSummaryBox type="condition" data={cond} />
          </div>
          <div className="lg:col-span-4">{cond.clinicalReview && (<div className="p-6 bg-brand-900 border border-default rounded-3xl text-white space-y-3 relative overflow-hidden"><p className="text-[10px] font-black uppercase tracking-widest text-emerald-300">Clinical Verification</p><p className="text-sm font-bold">{cond.clinicalReview.reviewedBy}</p><p className="text-[10px] font-mono opacity-50 uppercase border-t border-white/10 pt-2">Updated: {cond.clinicalReview.lastUpdated}</p></div>)}</div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            {activeTab === 'overview' && (<div className="animate-in fade-in card-premium p-8 md:p-12 space-y-8">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="p-6 bg-canvas border border-default rounded-[24px] space-y-3"><span className="text-[10px] font-black uppercase tracking-widest text-emerald-800">Target Biomarkers</span><ul className="space-y-2">{cond.biomarkers.map(b => (<li key={b} className="text-sm font-semibold text-stone-600 flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>{b}</li>))}</ul></div>
                <div className="p-6 bg-canvas border border-default rounded-[24px] space-y-3"><span className="text-[10px] font-black uppercase tracking-widest text-emerald-800">Therapeutic Spices</span><div className="flex flex-wrap gap-2 pt-1">{cond.therapeuticSpices.map(spice => { const match = herbsData.find(h => h.name.toLowerCase().includes(spice.split(' ')[0].toLowerCase())); return match ? (<CompatLink href={`/herb/${match.id}`} key={spice} className="px-3 py-1 bg-emerald-50 hover:bg-emerald-100 border border-emerald-100/50 text-emerald-800 text-xs font-black uppercase rounded-lg transition-colors">{spice} ↗</CompatLink>) : (<span key={spice} className="px-3 py-1 bg-stone-100 text-stone-600 text-xs font-bold rounded-lg">{spice}</span>); })}</div></div>
              </div>
              <div className="p-6 bg-accent border border-default rounded-3xl space-y-2"><div className="text-xs font-black uppercase tracking-widest text-emerald-950">Circadian Guidance</div><p className="text-sm text-stone-600 leading-relaxed">{cond.circadianAdvice}</p></div>
            ...
            {activeTab === 'biomarkers' && <div id="clinical-evidence-table"><BiomarkerRiskTracker biomarkersList={cond.biomarkers} /></div>}
            {activeTab === 'faqs' && (<div className="animate-in fade-in space-y-6">{(cond.faqs || []).map(faq => (<div key={faq.question} className="card-standard space-y-3"><h4 className="text-lg font-bold text-emerald-950 mb-3">{faq.question}</h4><p className="text-sm text-stone-600 leading-relaxed">{faq.answer}</p></div>))}</div>)}
          </div>
          <div className="lg:col-span-4 space-y-6">
            <div className="card-standard p-4 rounded-[40px]"><div className="aspect-[4/3] rounded-[28px] overflow-hidden mb-6 bg-emerald-950 relative flex items-center justify-center"><IconFlask className="w-24 h-24 text-lime-400/30" /></div><div className="p-4"><p className="text-xs text-stone-500 leading-relaxed mb-6">Optimize your circadian biology. Get a personalized protocol.</p><CompatLink href="/plans" className="w-full btn-primary py-4 text-center block">Start Your Program</CompatLink></div></div>
            <div className="card-standard bg-gradient-to-br from-emerald-50 to-lime-50/30 p-6 space-y-4"><div className="flex items-center gap-3"><span className="p-2 bg-emerald-600 text-white rounded-xl"><IconMapPin size={18} /></span><div><div className="text-sm font-bold text-emerald-950 brand-font">Outreach & Service Areas</div><p className="text-[9px] text-emerald-700 font-black uppercase tracking-wider">1,040+ Cities</p></div></div><CompatLink href="/clinics" className="w-full btn-secondary py-3.5 flex items-center justify-center gap-2 text-center block">Find a Clinic <IconArrowRight size={12} /></CompatLink></div>
          </div>
        </div>
...
export const ClinicalGenomicsDetail = ({ variant }: { variant: GenomicVariantEntity }) => (
  <div className="animate-in fade-in duration-700 min-h-screen bg-stone-50/30 pb-24 pt-32">
    <div className="max-w-7xl mx-auto px-6">
      ...
      <div className="grid lg:grid-cols-12 gap-12 mb-12"><div className="lg:col-span-8 space-y-6">
          {/* Breadcrumb label */}
          <div className="flex items-center gap-3 flex-wrap">
            <span className="badge-clinical">Nutrigenomics · SNP Profile</span>
            <span className={`badge-clinical ${variant.genotype.includes('High Risk') ? 'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300' : variant.genotype.includes('Moderate') ? 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300' : 'bg-emerald-50 text-emerald-700'}`}>{variant.genotype.includes('High Risk') ? '⚠ High Risk Variant' : variant.genotype.includes('Moderate') ? '⚡ Moderate Risk' : '✓ Low Risk Variant'}</span>
          </div>
          {/* Clear human-readable h1: Gene name */}
          <h1 className="text-5xl md:text-7xl font-black text-emerald-950 brand-font leading-none tracking-tight">{variant.geneName} Gene</h1>
          {/* Subtitle: what this gene does clinically */}
          <p className="text-lg text-stone-600 leading-relaxed max-w-2xl">{variant.clinicalImpact}</p>
          {/* Technical metadata row — secondary, clearly labelled */}
          <div className="flex flex-wrap gap-4 pt-2">
            <div className="flex items-center gap-2 px-4 py-2 bg-canvas rounded-xl border border-default">
              <span className="text-[9px] font-black uppercase tracking-widest text-stone-400">SNP ID</span>
              <span className="text-sm font-mono font-bold text-stone-700">{variant.rsid}</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-canvas rounded-xl border border-default">
              <span className="text-[9px] font-black uppercase tracking-widest text-stone-400">Your Genotype</span>
              <span className="text-sm font-mono font-bold text-stone-700">{variant.genotype.split(' ')[0]}</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-canvas rounded-xl border border-default">
              <span className="text-[9px] font-black uppercase tracking-widest text-stone-400">Gene</span>
              <span className="text-sm font-mono font-bold text-emerald-800">{variant.geneName}</span>
            </div>
          </div>
        </div></div>
      <div className="grid lg:grid-cols-12 gap-12">
        <div className="lg:col-span-8"><div className="card-premium p-8 md:p-12 space-y-8"><h2 className="text-2xl font-black text-emerald-950 brand-font">Dietary Swap Guidelines</h2><p className="text-stone-600 leading-relaxed">{variant.dietaryGuideline}</p>
          <div className="grid md:grid-cols-2 gap-6"><div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><h3 className="text-xs font-black uppercase tracking-widest text-emerald-800">Key Nutrients</h3><ul className="list-disc pl-4 text-sm text-stone-600 space-y-1">{variant.keyNutrients.map((n, i) => <li key={i}>{n}</li>)}</ul></div><div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><h3 className="text-xs font-black uppercase tracking-widest text-red-800">Foods to Avoid</h3><ul className="list-disc pl-4 text-sm text-stone-600 space-y-1">{variant.avoidFoods.map((f, i) => <li key={i}>{f}</li>)}</ul></div></div>
          <div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><h3 className="text-xs font-black uppercase tracking-widest text-emerald-800">Target Biomarkers</h3><div className="flex flex-wrap gap-2">{variant.targetBiomarkers.map((b, i) => (<span key={i} className="px-3 py-1 bg-emerald-50 text-emerald-800 rounded-lg text-xs font-bold border border-emerald-100">{b}</span>))}</div></div>
        </div></div>
        <div className="lg:col-span-4"><div className="p-8 bg-accent border border-default rounded-3xl space-y-6"><h2 className="text-lg font-black text-emerald-950 uppercase tracking-tight">Clinical Evidence</h2>{variant.citations.map((cite, idx) => (<div key={idx} className="text-xs space-y-1 border-l-2 border-emerald-200 pl-3"><p className="font-semibold text-stone-700">{cite.text}</p><a href={cite.url} target="_blank" rel="noopener noreferrer" className="text-emerald-800 hover:underline block truncate">PubMed Link →</a></div>))}</div></div>
      </div>
...
export const ClinicalInteractionDetail = ({ interaction }: { interaction: DrugInteractionEntity }) => (
  <div className="animate-in fade-in duration-700 min-h-screen bg-stone-50/30 pb-24 pt-32">
    <div className="max-w-7xl mx-auto px-6">
      <CompatLink href="/sitemap" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-emerald-800 hover:text-emerald-600 mb-8">← Back</CompatLink>
      <div className="grid lg:grid-cols-12 gap-12 mb-12"><div className="lg:col-span-8 space-y-6"><div className="flex items-center gap-3"><span className="badge-clinical">Drug-Nutrient Interaction</span><span className="text-xs font-mono text-stone-500 italic">Class: {interaction.drugClass}</span></div><h1 className="text-5xl md:text-7xl font-black text-emerald-950 brand-font leading-none tracking-tight">{interaction.drugName}</h1><p className="text-lg text-stone-600 leading-relaxed max-w-2xl">{interaction.monitoringAdvice}</p></div></div>
      <div className="grid lg:grid-cols-12 gap-12">
        <div className="lg:col-span-8"><div className="card-premium p-8 md:p-12 space-y-8"><h2 className="text-2xl font-black text-emerald-950 brand-font">Interaction Profile</h2>
          <div className="grid md:grid-cols-3 gap-6"><div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><h3 className="text-xs font-black uppercase tracking-widest text-red-800">Depleted Nutrients</h3><ul className="list-disc pl-4 text-sm text-stone-600 space-y-1">{interaction.depletedNutrients.map((d, i) => <li key={i}>{d}</li>)}</ul></div><div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><h3 className="text-xs font-black uppercase tracking-widest text-amber-800">Contraindicated Herbs</h3><ul className="list-disc pl-4 text-sm text-stone-600 space-y-1">{interaction.contraindicatedHerbs.map((c, i) => <li key={i}>{c}</li>)}</ul></div><div className="p-6 bg-canvas border border-default rounded-[24px] space-y-2"><h3 className="text-xs font-black uppercase tracking-widest text-emerald-800">Synergistic Herbs</h3><ul className="list-disc pl-4 text-sm text-stone-600 space-y-1">{interaction.synergisticHerbs.map((s, i) => <li key={i}>{s}</li>)}</ul></div></div>
        </div></div>
