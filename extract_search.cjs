...
const localDirFile = path.join(__dirname, 'src/components/LocalDirectory.tsx');
let content = fs.readFileSync(localDirFile, 'utf-8');

const locationSearchCode = `import React, { useState, useMemo } from 'react';
...
export const LocationSearch: React.FC<{ navigate: (path: string) => void }> = ({ navigate }) => {
  const { trackInteraction } = useViewerTracker('LocationSearch');
...
fs.writeFileSync(path.join(__dirname, 'src/components/LocationSearch.tsx'), locationSearchCode);
console.log('LocationSearch.tsx created.');
