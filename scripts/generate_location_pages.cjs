// scripts/generate_location_pages.cjs
// Generates rich, SEO/AEO/GEO/AIO-optimised markdown pages for every city×disease
// combination found in LOCATIONS_DATA.
// Run: node scripts/generate_location_pages.cjs
// Dry-run: node scripts/generate_location_pages.cjs --dry
...
function excerpt(loc, disease) {
  return `Evidence-based nutrition guidance for managing ${disease} in ${loc.city}, ${loc.state}. ` +
    `Local staples, therapeutic spices, and circadian meal timing tailored to the ${loc.weather.split(',')[0].toLowerCase()} climate.`;
}

function buildContent(loc, disease, slug) {
  const title       = `${disease} Nutrition Guide – ${loc.city}, ${loc.state}`;
  const desc        = excerpt(loc, disease);
  const kws         = keywords(loc, disease);
  const url         = `${SITE}/locations/${slug}`;
  const now         = new Date().toISOString().split('T')[0];

  // Geo coords placeholder (real data not in source; mark for later enrichment)
        ...
        "mainEntity": [
          {
            ...
            "acceptedAnswer": {
              "@type": "Answer",
              "text": `In ${loc.city}, locally available ${loc.localFruits.slice(0,2).join(' and ')} combined with therapeutic ${loc.localTherapeuticSpice} can support ${disease} management through their synergistic phytonutrient profiles.`
            }
          },
          {
            "@type": "Question",
          ...
          },
          {
            ...
            "acceptedAnswer": {
              "@type": "Answer",
              "text": `${loc.regionalDeficiencyRisk} is a prevalent deficiency in ${loc.city} that can worsen ${disease}. Targeted supplementation and local food choices can help address this gap.`
            }
          }
        ]
      }
  ...
  const fm = [
    ...
    ...kws.map(k => `  - "${k}"`),
    `geo:`,
    `  city: "${escape(loc.city)}"`,
    `  state: "${escape(loc.state)}"`,
    `  country: "India"`,
    `  region: "${geoRegion}"`,
    `  pincode: "${loc.pincode}"`,
    `robots: "index, follow"`,
  ...
  // ── page body ─────────────────────────────────────────────────────────────
  const localFruits = loc.localFruits.join(', ');
  const localVeg    = loc.localVegetables.join(', ');
  const famous      = loc.famousProduce.join(', ');

  const body = `# ${title}

> **Clinical target:** ${loc.clinicalTargetBiomarker}
...
The ${loc.microclimateMetabolicImpact.toLowerCase()}
...
| Category | Foods |
|----------|-------|
| **Local Fruits** | ${localFruits} |
| **Local Vegetables** | ${localVeg} |
| **Famous Produce** | ${famous} |
...
Aligning your meal timing with your body's natural clock is especially important in ${loc.city}'s ${loc.weather.split(',')[0].toLowerCase()} climate. Aim to:
...
// ── main ─────────────────────────────────────────────────────────────────────
async function main() {
  // Dynamically import the ES-module locationsData
  const dataPath = path.resolve(__dirname, '../src/lib/locationsData.js');
  let LOCATIONS_DATA;
  try {
    const mod = await import(dataPath);
    LOCATIONS_DATA = mod.LOCATIONS_DATA;
  } catch (e) {
    // Fallback: try .ts compiled output path
    console.error('Could not import locationsData.js, trying alternative path…', e.message);
    process.exit(1);
  }

  if (!LOCATIONS_DATA || !LOCATIONS_DATA.length) {
    console.error('LOCATIONS_DATA is empty or undefined. Check the import path.');
    process.exit(1);
  }
  ...
  for (const loc of LOCATIONS_DATA) {
    ...
    for (const disease of healthIssues) {
      ...
      if (DRY) {
        log.push(`[DRY] Would write → ${filePath}`);
      } else {
  ...
  if (DRY) {
    const logFile = path.resolve(__dirname, 'generate_location_pages_dryrun.log');
    fs.writeFileSync(logFile, log.join('\n'), 'utf8');
    console.log(`[DRY-RUN] Would generate ${count} pages. Log → ${logFile}`);
  } else {
