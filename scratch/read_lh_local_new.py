...
print("\nUnused JS score:", audit.get('score'))
print("Unused JS display value:", audit.get('displayValue'))
details = audit.get('details', {})
if details.get('type') == 'opportunity':
    items = details.get('items', [])
    print("Unused JS items:")
    for item in items:
        print(f"  * {item.get('url')} (Total: {item.get('totalBytes', 0)/1024:.1f} KB, Wasted: {item.get('wastedBytes', 0)/1024:.1f} KB)")
