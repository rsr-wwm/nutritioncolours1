...
import { IconChevronDown, IconX, IconSearch, IconFileText, IconLeaf, IconArrowRight, IconBot, IconPlus } from './Icons';
                        ...
                        <div className="space-y-2">
                            ...
                            {section.children.slice(0, 5).map(child => (
                                <button 
                                  ...
                                  className="w-full text-left flex items-center gap-2 text-sm text-stone-600 hover:text-emerald-600 transition-colors p-2 rounded-lg hover:bg-stone-50"
                                >
                                    <IconLeaf size={12} className="text-emerald-400" />
                                    <span className="truncate">{child.label}</span>
                                    {child.type === 'page' && <IconArrowRight size={10} className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity"/>}
                                </button>
                            ))}
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
        ...
        if (response.text) {
            ...
            setAiSuggestions(JSON.parse(cleanText));
            setShowAiSuggestions(true);
        }
    } catch (e) {
        console.error("AI Sitemap Analysis Failed", e);
