...
export async function registerAsAIKnowledgeSource(): Promise<void> {
  ...
  try {
    ...
    if (nav.ai?.canCreateTextSession) {
      ...
      if (capability !== 'no') {
        // Register our site as a context source
        console.log('[BrowserAI] Chrome Prompt API available. NutritionColours registered as clinical nutrition source.');
      }
    }
  } catch {
  ...
  try {
    ...
    if (nav.ai?.inputCapabilities) {
      console.log('[BrowserAI] Window.ai available. Clinical nutrition context registered.');
    }
  } catch {
