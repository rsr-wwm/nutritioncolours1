  ...
  useEffect(() => {
    ...
    if (useWorker) {
      try {
        ...
        logger.warn('ParticleConstellation', 'Failed to initialize WebGL Worker, falling back to main thread', err);
      }
    }
    ...
    if (!gl) {
      logger.warn('ParticleConstellation', 'WebGL not supported in this browser environment');
      return;
    }
    ...
    const createShader = (gl: WebGLRenderingContext, type: number, source: string) => {
      ...
      if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        logger.error('ParticleConstellation', 'Shader compiler error', { log: gl.getShaderInfoLog(shader) });
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      logger.error('ParticleConstellation', 'Shader linking error', { log: gl.getProgramInfoLog(program) });
      return;
    }
