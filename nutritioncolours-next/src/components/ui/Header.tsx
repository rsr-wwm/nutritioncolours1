...
export function Header() {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-50 bg-white/80 dark:bg-stone-950/80 backdrop-blur-md border-b border-stone-200/50 transition-all duration-300 shadow-sm" data-nav-context={navContext}>
        {/* Top Info Bar */}
        <div className="bg-emerald-950 text-white py-1.5 border-b border-emerald-900/30">
            ...
            {NAV_CONFIG.map(item => {
              ...
              return (
                <div key={item.id} className="relative group">
                  <CompatLink
                    href={item.children ? '#' : `/${item.id === 'home' ? '' : item.id}`}
