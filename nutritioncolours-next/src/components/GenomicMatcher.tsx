...
export const GenomicMatcher = ({ activeLocation }: { activeLocation?: any }) => {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      // Consent gate for genomic data (highest sensitivity)
      if (!ensureAIConsent('genomic')) {
        grantAIConsent('genomic');
        alert(getConsentNotice('genomic') + '\n\nBy continuing, you acknowledge this notice.');
      }
      ...
      if (response.text) {
        const data = safeJsonParse<any>(response.text.replace(/```json\n?|\n?```/g, '').trim(), null);
      ...
      logger.error('GenomicMatcher', 'DNA data parsing failed', e);
